...
5. Влад
6. Санечка
7. МАКСОН (логин: `max`, пароль: `MaxonChinazes`)

Если база уже была создана до добавления нового пользователя, выполни один раз из backend: `npm run seed` (при запущенном Docker — из контейнера backend или с хоста с `DATABASE_URL` на контейнер БД).
...
### Добавление пользователя в уже существующую БД (после git pull)

Если контейнер БД уже создан, `init.sql` не выполняется повторно. Чтобы добавить или обновить пользователя МАКСОН (логин `max`), один раз выполни seed:

```bash
docker compose exec backend npm run seed
```

Либо с хоста (если backend не в Docker): из папки `backend` с `DATABASE_URL` на твою БД: `npm run seed`.
